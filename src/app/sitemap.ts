import type { MetadataRoute } from "next";
import { asc, desc, eq } from "drizzle-orm";
import { blogPosts, db, products } from "@/db";

export const revalidate = 3600;

const baseUrl = "https://codeddevs.com";

const staticRoutes = [
  "",
  "/products",
  "/blog",
  "/team",
];

export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const now = new Date();
  const staticEntries = staticRoutes.map((route) => ({
    url: `${baseUrl}${route}`,
    lastModified: now,
  }));

  const [postEntries, productEntries] = await Promise.all([
    (async () => {
      try {
        return await db
          .select({
            slug: blogPosts.slug,
            updated_at: blogPosts.updated_at,
          })
          .from(blogPosts)
          .where(eq(blogPosts.is_published, true))
          .orderBy(desc(blogPosts.published_at));
      } catch (error) {
        console.error("[sitemap] blog post entries failed:", error);
        return [];
      }
    })(),
    (async () => {
      try {
        return await db
          .select({
            slug: products.slug,
            updated_at: products.updated_at,
          })
          .from(products)
          .orderBy(asc(products.order_index));
      } catch (error) {
        console.error("[sitemap] product entries failed:", error);
        return [];
      }
    })(),
  ]);

  return [
    ...staticEntries,
    ...productEntries.map((product) => ({
      url: `${baseUrl}/products/${product.slug}`,
      lastModified: product.updated_at,
    })),
    ...postEntries.map((post) => ({
      url: `${baseUrl}/blog/${post.slug}`,
      lastModified: post.updated_at,
    })),
  ];
}
